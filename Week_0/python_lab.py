...
square_dict = {(x):x**2 for x in range(99)}

##Task 0.5.24 
{i:j for i in range(len(D)) for j in D}

## 23: (Task 23) Making the identity function
D = {'red','white','blue'}
# Replace {...} with a one-line dictionary comprehension
identity_dict = {i for i in range(len(D))}
...
def all_3_digit_numbers(base, digits): return range(len([[i,j,k] for i in digits for j in digits for k in digits]))